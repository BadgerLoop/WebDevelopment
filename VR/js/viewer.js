...
if (mobile){ // Use Gyro-sensor for mobile phones
	...
	controls.connect();
	controls.update();
	var vr = confirm('Enable VR Headset Mode?'); // Ask user
...
if (vr){
	var pseudo_stereo = new THREE.StereoEffect(renderer);
}
...
function resize() {
	...
	if (vr){
		pseudo_stereo.setSize(window.innerWidth, window.innerHeight);
	}
}
...
function render(dt){
    ...
    if (vr){
		pseudo_stereo.render(scene, camera);
	}
}
